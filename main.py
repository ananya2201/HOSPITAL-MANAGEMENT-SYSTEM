...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost/hospital_management_system'
db=SQLAlchemy(app)
...
def patient():
    ...
    if request.method=="POST":
        ...
        query=db.engine.execute(f"INSERT INTO `appointments` (`email`,`name`,	`gender`,`slot`,`disease`,`time`,`date`,`dept`,`number`) VALUES ('{email}','{name}','{gender}','{slot}','{disease}','{time}','{date}','{dept}','{number}')")
